...
var (
	oauthStatusMu sync.RWMutex
	oauthStatus   = make(map[string]string)
)

// getOAuthStatus safely retrieves the OAuth status for a given state.
func getOAuthStatus(state string) string {
	oauthStatusMu.RLock()
	defer oauthStatusMu.RUnlock()
	return oauthStatus[state]
}

// setOAuthStatus safely sets the OAuth status for a given state.
func setOAuthStatus(state, value string) {
	oauthStatusMu.Lock()
	defer oauthStatusMu.Unlock()
	oauthStatus[state] = value
}

// deleteOAuthStatus safely deletes the OAuth status for a given state.
func deleteOAuthStatus(state string) {
	oauthStatusMu.Lock()
	defer oauthStatusMu.Unlock()
...
func (h *Handler) GetAuthStatus(c *gin.Context) {
	state := c.Query("state")
	oauthStatusMu.RLock()
	err, ok := oauthStatus[state]
	oauthStatusMu.RUnlock()
	if ok {
		if err != "" {
