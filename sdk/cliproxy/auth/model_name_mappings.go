...
func compileModelNameMappingTable(mappings map[string]map[string]sdkconfig.ModelNameMapping) *modelNameMappingTable {
	if len(mappings) == 0 {
	...
	for rawChannel, entriesMap := range mappings {
		channel := strings.ToLower(strings.TrimSpace(rawChannel))
		if channel == "" || len(entriesMap) == 0 {
			continue
		}
		rev := make(map[string]string, len(entriesMap))
...
func (m *Manager) SetOAuthModelMappings(mappings map[string]map[string]sdkconfig.ModelNameMapping) {
	if m == nil {
		return
	}
