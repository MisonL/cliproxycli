...
import { IconTrash2, IconRefreshCw, IconPlay, IconSearch, IconFilter, IconPause, IconScrollText, IconClock, IconCalendar, IconCpu, IconActivity, IconPlus } from '@/components/ui/icons';
...
export function SchedulerPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      showNotification(`${t('notification.refresh_failed')}: ${(err as Error)?.message || ''}`, 'error');
    } finally {
      setLoading(false);
    }
  }, [showNotification, t]);
  ...
  const handleRunTask = useCallback(async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
  ...
  const filteredLogs = logs.filter(log => {
    const matchSearch = log.task_name.toLowerCase().includes(logSearchQuery.toLowerCase()) ||
  ...
  const handleBatchDelete = async () => {
    if (!confirm(t('scheduler.messages.confirm_batch_delete') || `Delete ${selectedTasks.size} tasks?`)) return;
    try {
  ...
  const handleBatchStatus = async (status: 'active' | 'paused') => {
    try {
      await Promise.all(Array.from(selectedTasks).map(id => schedulerApi.updateTask(id, { status })));
      showNotification(t('scheduler.task_updated'), 'success');
      loadData();
    } catch (err: unknown) {
      showNotification(`${t('scheduler.save_failed')}: ${(err as Error)?.message}`, 'error');
  ...
  const stats = {
      ...
      successRate: logs.length > 0 ? Math.round((logs.filter(l => l.success).length / logs.length) * 100) : 100
  };
    ...
    <div className={styles.container}>
      ...
      <div className={styles.statsBar}>
        <div className={`${styles.statCard} ${styles.pending}`}>
          <span className={styles.statLabel}>{t('common.total_tasks', { defaultValue: 'Total Tasks' })}</span>
        ...
        <div className={`${styles.statCard} ${styles.success}`}>
          <span className={styles.statLabel}>{t('scheduler.success_rate', { defaultValue: '24h Success Rate' })}</span>
      ...
      <div className={styles.toolbar}>
        <div className={styles.toolbarLeft}>
          ...
          <div style={{ position: 'relative' }}>
             <IconFilter size={16} style={{ position: 'absolute', left: 12, top: 11, pointerEvents: 'none', color: 'var(--text-secondary)' }} />
             <select 
        ...
        <div className={styles.actions}>
           {selectedTasks.size > 0 && (
              <div className={styles.batchActions}>
                 <Button size="sm" variant="secondary" onClick={() => handleBatchStatus('paused')}><IconPause size={14} /></Button>
                 <Button size="sm" variant="secondary" onClick={() => handleBatchStatus('active')}><IconPlay size={14} /></Button>
                 <Button size="sm" variant="danger" onClick={handleBatchDelete}><IconTrash2 size={14} /></Button>
              </div>
           )}
      ...
      <div className={styles.taskSection}>
        {loading && tasks.length === 0 ? (
          <div className="flex-center" style={{ padding: '60px' }}><LoadingSpinner /></div>
          ...
          <div className={styles.taskGrid}>
            {filteredTasks.map(task => (
              <div key={task.id} className={`${styles.taskCard} ${selectedTasks.has(task.id) ? styles.selected : ''}`}>
                <div className={styles.cardHeader}>
                  ...
                  <ToggleSwitch 
                    ...
                    onChange={(checked) => handleStatusChange(task.id, checked ? 'active' : 'paused')}
                  />
                </div>
                ...
                <div className={styles.cardBody}>
                  <div className={styles.metaRow}>
                    <IconClock size={14} />
                    <span>
                      {task.type === 'interval' ? `${t('scheduler.meta.every')} ${task.interval}` : 
                       task.type === 'daily' ? `${t('scheduler.meta.daily_at')} ${task.daily_time}` :
                  ...
                  <div className={styles.metaRow}>
                    ...
                    <span>{t('scheduler.meta.next_run')}: {task.next_run_at ? new Date(task.next_run_at).toLocaleString() : '-'}</span>
                  </div>
                </div>

                <div className={styles.cardFooter}>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <Button variant="ghost" size="sm" onClick={(e) => handleRunTask(task.id, e)} title={t('scheduler.run_now')}>
                      <IconPlay size={16} />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleViewTaskLogs(task.id)} title={t('scheduler.view_logs')}>
                      <IconScrollText size={16} />
                    </Button>
                  </div>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <Button variant="secondary" size="sm" onClick={() => handleEdit(task)}>{t('scheduler.edit')}</Button>
      ...
      <div className={styles.logsSection}>
        <div className={styles.logsHeader}>
          ...
          <div className={styles.toolbarLeft}>
            ...
            />
            <Button variant="ghost" size="sm" onClick={handleClearLogs} disabled={logs.length === 0}>
               <IconTrash2 size={16} />
            </Button>
          </div>
        </div>
              ...
              <tbody>
                 {filteredLogs.slice(0, 50).map(log => (
                     <tr key={log.id}>
                       ...
                       <td className={styles.logOutputCell} title={log.output} onClick={() => {
                 ...
                 {filteredLogs.length === 0 && (
                    <tr><td colSpan={5} style={{ textAlign: 'center', padding: '32px', color: 'var(--text-secondary)' }}>{t('scheduler.no_logs')}</td></tr>
                 )}
              </tbody>
           </table>
        </div>
      ...
      <Modal 
        ...
        footer={
          <>
            <Button variant="secondary" onClick={() => setModalOpen(false)}>{t('scheduler.cancel')}</Button>
            <Button onClick={handleSave} loading={saving}>{t('scheduler.save')}</Button>
          </>
        }
      >
        ...
        <div className={styles.formGrid}>
          <div className={styles.fullWidth}>
            <div className={styles.formSectionTitle}><IconActivity size={14} /> {t('common.basic_info', { defaultValue: 'Basic Information' })}</div>
            <Input
          ...
          <div className={styles.fullWidth}>
            <div className={styles.formSectionTitle}><IconClock size={14} /> {t('scheduler.scheduling', { defaultValue: 'Scheduling' })}</div>
               ...
               <div className={styles.fullWidth}>
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        if (tempTimePoint && !formData.daily_time.includes(tempTimePoint)) {
                          setFormData({...formData, daily_time: [...formData.daily_time, tempTimePoint].sort()});
                        }
                      }}
                    >
                 ...
                 <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
                    {formData.daily_time.map(time => (
                      <span key={time} style={{ background: 'var(--bg-hover)', padding: '4px 10px', borderRadius: '6px', fontSize: '13px', display: 'flex', alignItems: 'center', gap: 6, border: '1px solid var(--border-color)' }}>
                        {time}
                        <IconTrash2 size={12} style={{ cursor: 'pointer', color: 'var(--error-color)' }} onClick={() => setFormData({...formData, daily_time: formData.daily_time.filter(t => t !== time)})} />
                      </span>
                    ))}
                    {formData.daily_time.length === 0 && <span style={{ fontSize: '12px', color: 'var(--text-tertiary)' }}>No time points added</span>}
                 </div>
               </div>
            ) : (
               <Input
          ...
          <div className={styles.fullWidth}>
            <div className={styles.formSectionTitle}><IconCpu size={14} /> {t('scheduler.ai_config', { defaultValue: 'AI Configuration' })}</div>
          </div>
      ...
      >
        <div style={{ maxHeight: '70vh', overflowY: 'auto' }}>
            {logs.filter(l => l.task_id === logTaskFilter).length === 0 ? (
                <p style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>{t('scheduler.no_history_for_task')}</p>
            ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {logs.filter(l => l.task_id === logTaskFilter).map(log => (
                        <div key={log.id} style={{ padding: '16px', background: 'var(--bg-hover)', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: '13px', fontWeight: 500 }}>
                                <span style={{ color: 'var(--text-secondary)' }}>{new Date(log.executed_at).toLocaleString()}</span>
                                <span className={log.success ? styles.statusSuccess : styles.statusError}>
                                    {log.success ? t('scheduler.log_success') : t('scheduler.log_failed')} ({formatDuration(log.duration_ms)})
                                </span>
                            </div>
                            <pre style={{ margin: 0, whiteSpace: 'pre-wrap', fontSize: '13px', lineHeight: 1.5, opacity: 0.9 }}>
                                {log.output}
                            </pre>
                        </div>
                    ))}
